...
async function executeTurn(command: ExecuteCommand): Promise<void> {
  ...
  try {
    ...
    if (session === null) {
      ...
      session = (["vc-agent-faux", "vc-agent-memory-faux", "vc-agent-explicit-memory-faux", "vc-agent-reflection-faux", "vc-agent-reflection-memory-faux", "vc-agent-unscoped-evidence-faux", "vc-agent-unscoped-memory-faux", "vc-agent-dream-synthesis-faux"].includes(command.profile.provider)) && process.env.NODE_ENV === "test"
