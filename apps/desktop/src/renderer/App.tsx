        ...
        {view === "settings" ? (
          ...
          <section className="conversation" aria-label="Conversation">
            ...
            <div className="message-list">
              {activeReflection !== undefined && <ReflectionWorkspace run={activeReflection} outcomes={reflectionOutcomes[activeReflection.id] ?? { judgments: [], learningProposals: [] }} profiles={profiles} taskAssignments={taskAssignments} start={startOrRetryReflection} startMemoryAware={startMemoryAwareReflection} stop={() => void invoke(createCommand({ command: "reflection.independent.stop", payload: { runId: activeReflection.id } }))} discard={() => void invoke(createCommand({ command: "reflection.discard", payload: { runId: activeReflection.id } }))} configure={() => setView("settings")} prepareOutcomes={() => submit("Prepare a Judgment Record and one de-identified Long-term Learning Proposal from this Reflection.")} confirmJudgment={(draftId) => void invoke(createCommand({ command: "reflection.judgment.confirm", payload: { draftId } }))} discardOutcome={(draftId) => void invoke(createCommand({ command: "reflection.outcome.discard", payload: { draftId } }))} prepareLearningPatch={(proposalId, judgmentDraftId) => void invoke(createCommand({ command: "reflection.learning.prepare_patch", payload: { proposalId, judgmentDraftId } }))} />}
...
function ReflectionWorkspace({ run, outcomes, profiles, taskAssignments, start, startMemoryAware, stop, discard, configure, prepareOutcomes, confirmJudgment, discardOutcome, prepareLearningPatch }: { run: ReflectionRun; outcomes: { judgments: JudgmentRecordDraft[]; learningProposals: LongTermLearningProposal[] }; profiles: ModelProfile[]; taskAssignments: TaskModelAssignment[]; start(run: ReflectionRun, profileId?: string): void; startMemoryAware(run: ReflectionRun, profileId?: string): void; stop(): void; discard(): void; configure(): void; prepareOutcomes(): void; confirmJudgment(draftId: string): void; discardOutcome(draftId: string): void; prepareLearningPatch(proposalId: string, judgmentDraftId: string): void }) {
  ...
  return <article className="reflection-workspace" data-testid="reflection-workspace">
    ...
    {outcomes.judgments.map((draft) => <section className="reflection-outcome" data-testid="judgment-record-draft" key={draft.id}><div><span>{draft.status === "draft" ? "Draft · not authoritative" : draft.status}</span><h3>Judgment Record</h3></div><p>{draft.view}</p><dl><div><dt>Decision</dt><dd>{draft.decisionState}</dd></div><div><dt>Sources</dt><dd>{draft.sourceAvailability}</dd></div></dl>{draft.status === "draft" && run.status === "dialogue_active" && <div className="form-actions"><button type="button" onClick={() => discardOutcome(draft.id)}>Discard</button><button className="primary-button" type="button" onClick={() => confirmJudgment(draft.id)}>Confirm Judgment Record</button></div>}</section>)}
    {outcomes.learningProposals.map((proposal) => <section className="reflection-outcome learning-proposal" data-testid="learning-proposal-draft" key={proposal.id}><div><span>{proposal.status === "draft" ? "Draft · not in Memory" : proposal.status.replace("_", " ")}</span><h3>Long-term Learning Proposal</h3></div><strong>{proposal.proposed.title}</strong><p>{proposal.proposed.content}</p><p>{proposal.comparisonSummary}</p>{proposal.status === "draft" && run.status === "dialogue_active" && <div className="form-actions"><button type="button" onClick={() => discardOutcome(proposal.id)}>Discard</button><button className="primary-button" type="button" disabled={confirmedJudgment === undefined} onClick={() => confirmedJudgment && prepareLearningPatch(proposal.id, confirmedJudgment.id)}>Preview Memory Patch</button></div>}</section>)}
