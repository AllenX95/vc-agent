...
test("completes the desktop C1 fixture paths without eager external activation", async () => {
  ...
  writeFileSync(join(extensionSource, "index.js"), "module.exports = {};", "utf8");
  const root = resolve(import.meta.dirname, "../..");
  const application = await launchApplication(root, userDataDirectory, { VC_AGENT_TEST_PROJECT_PATH: projectDirectory, VC_AGENT_TEST_SKILL_SOURCE: skillSource, VC_AGENT_TEST_EXTENSION_SOURCE: extensionSource, VC_AGENT_TEST_OFFICE_SOURCE_PATH: join(projectDirectory, "fixture.docx") });
  try {
    const window = await application.firstWindow();
    ...
    const profile = await invokeRaw(window, "profile.create", { name: "C1 fixture", provider: "vc-agent-faux", model: "fixture", apiKey: "fixture-key", thinkingLevel: "off" }) as { payload: { profile: { id: string } } };
    await invokeRaw(window, "thread.profile.select", { threadId: projectThread!.id, profileId: profile.payload.profile.id });
    await window.reload();
    ...
    await officeCard.getByLabel("Office operation").selectOption("edit");
    await officeCard.getByRole("button", { name: "Choose source" }).click();
    await expect(officeCard.getByLabel("Office source path")).toHaveValue(join(projectDirectory, "fixture.docx"));
    await officeCard.getByRole("button", { name: "Prepare Office task" }).click();
    await officeCard.getByRole("button", { name: "Run", exact: true }).click();
    ...
    await officeCard.getByRole("button", { name: "Confirm replacement" }).click();
    await officeCard.getByLabel("Office operation").selectOption("review");
    await officeCard.getByRole("button", { name: "Choose source" }).click();
    await officeCard.getByRole("button", { name: "Prepare Office task" }).click();
    await officeCard.getByRole("button", { name: "Run", exact: true }).click();
    await expect(officeCard.getByText("Change summary/diff ready").last()).toBeVisible();
    await expect(officeCard.getByText("Render/preview: ready").last()).toBeVisible();
    await integration.getByLabel("Skill package id").fill("c1-created-skill");
    await integration.getByLabel("Skill name").fill("C1 Created Skill");
    await integration.getByLabel("Skill description").fill("A C1 fixture draft");
    await integration.getByLabel("Skill objective").fill("Create a bounded C1 fixture skill.");
    await integration.getByRole("button", { name: "Create explicit draft" }).click();
    await expect(integration.getByText("c1-created-skill", { exact: false })).toBeVisible();
    await integration.getByRole("button", { name: "Review", exact: true }).click();
...
test("executes an explicitly configured Office runner through the Utility Worker", async () => {
  ...
  try {
    ...
    const projectThread = listedThreads.payload.threads.find((thread) => thread.scope === "project" && thread.projectId === projectId);
    expect(projectThread).toBeDefined();
    const profile = await invokeRaw(window, "profile.create", { name: "Office runner fixture", provider: "vc-agent-faux", model: "fixture", apiKey: "fixture-key", thinkingLevel: "off" }) as { payload: { profile: { id: string } } };
    await invokeRaw(window, "thread.profile.select", { threadId: projectThread!.id, profileId: profile.payload.profile.id });
    await window.reload();
    await window.getByRole("button", { name: "Thread 1", exact: true }).click();
    await expect(window.getByRole("combobox", { name: "Active Model Profile" })).toHaveValue(profile.payload.profile.id);
...
test("cancels a running Office runner and reports an interrupted job", async () => {
  ...
  try {
    ...
    const projectThread = listedThreads.payload.threads.find((thread) => thread.scope === "project" && thread.projectId === projectId);
    expect(projectThread).toBeDefined();
    const profile = await invokeRaw(window, "profile.create", { name: "Office cancellation fixture", provider: "vc-agent-faux", model: "fixture", apiKey: "fixture-key", thinkingLevel: "off" }) as { payload: { profile: { id: string } } };
    await invokeRaw(window, "thread.profile.select", { threadId: projectThread!.id, profileId: profile.payload.profile.id });
    await window.reload();
    await window.getByRole("button", { name: "Thread 1", exact: true }).click();
    await expect(window.getByRole("combobox", { name: "Active Model Profile" })).toHaveValue(profile.payload.profile.id);
    ...
    await expect(officeCard).toContainText("office:create · docx · interrupted", { timeout: 15_000 });
  } finally {
    await application.close();
    rmSync(userDataDirectory, { recursive: true, force: true });
    rmSync(projectDirectory, { recursive: true, force: true });
