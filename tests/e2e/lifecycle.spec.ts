...
test("keeps one desktop writer and exits the second instance", async () => {
  ...
  try {
    first = await electron.launch({ args: [join(root, "apps/desktop/dist/main/main.js"), `--user-data-dir=${userDataDirectory}`], cwd: root, env: testEnvironment({ VC_AGENT_USER_DATA_DIR: userDataDirectory }) });
    await expect(first.firstWindow()).resolves.toBeTruthy();
    second = spawn(first.process().spawnfile, [join(root, "apps/desktop/dist/main/main.js"), `--user-data-dir=${userDataDirectory}`], { cwd: root, env: testEnvironment({ VC_AGENT_USER_DATA_DIR: userDataDirectory }), windowsHide: true, stdio: "ignore" });
