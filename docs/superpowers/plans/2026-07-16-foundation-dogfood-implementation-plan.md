...
| PaddleOCR and future parsers | Material Pipeline, Parser Adapter registry, Canonical Parse | Native parsers emit parser-neutral blocks and warnings. OCR is a later page-recovery adapter behind the same pipeline; it must not require a second material model or a new model-visible tool. |
