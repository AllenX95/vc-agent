...
At proposal creation, the Host snapshots only locally verifiable dependencies that actually shaped the Reflection: exact Material references and content versions, Project or Long-term Memory entries recalled in the user-facing dialogue, and target Long-term Memory entries named by a Learning Proposal. A later change or disappearance of one of those exact dependencies deterministically marks the still-unconfirmed draft or unadopted proposal stale before listing, confirmation, or patch preparation. Unrelated Project or Memory edits do not invalidate it, and realtime web references are not silently refetched to perform a staleness check. A stale outcome remains inspectable and discardable but cannot be confirmed or promoted; the User must continue the Reflection and prepare a replacement. Confirmed Judgment Records and adopted learning are immutable historical decisions and never become stale retroactively.
...
For Investment Reflection, the Host records a bounded Reflection Dream Eligibility Signal only on active user-facing dialogue turns with an attributable User adoption, correction, or confirmation. The initial Memory-Aware model response, outcome-preparation commands without a User view, failed or interrupted turns, retries that do not complete, tool activity, and assistant text without such a User signal remain ineligible. A qualifying retry is eligible only through its own completed turn. The marker and exact Thread/Turn reference are persisted with `turn.submitted`; Dream selection requires the same Turn to have a durable `turn.completed`. The qualifying exchange may also enter the Short-term Memory Candidate queue, but neither marker nor candidate grants Memory authority before Dream review.
