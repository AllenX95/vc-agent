...
- Detect OCR need and invoke the local PaddleOCR Configured OCR Capability when available, using ordinary OCR by default and PP-StructureV3 only for complex layout, table, formula, or seal recovery.
...
4. Integration Build: the isolated VC Agent Skills Directory, compatible User-supplied Claude Office Skills, local PaddleOCR, pinned `pi-mcp-adapter`, compatible Skill Import, reused Skill Creator, and the complete Extension Admission Review, approval, Global Extension Revision, and rollback flow. Each integration must have a real task path and visible unavailable state without blocking unrelated daily work.
...
| OCR | Detect OCR need and invoke local PaddleOCR, using ordinary OCR by default and PP-StructureV3 on demand for complex pages | PaddleOCR/PaddlePaddle runtime and model installation under Access Mode policy | Clear unavailable/failure warning; parsed artifact marks OCR missing; no automatic switch to another OCR provider | Local-only by default; any future external OCR follows Access Mode and visible provenance |
...
- Bundled PaddleOCR, PaddlePaddle, CUDA, or Python OCR runtime.
...
runtimes/
  python/           pinned PyMuPDF and PaddleOCR environment and job handlers
```
...
| Tier | Work | Lifecycle |
| --- | --- | --- |
| Utility Worker Pool | Pinned vc-agent-owned PyMuPDF parsing and rendering, deterministic document parsing, PaddleOCR, derived-index rebuilds, and similar bounded local transforms | Lazy, bounded workers may remain warm while the application is open so heavy local models do not reload for every page. |
...
PDF parsing is one deterministic page-aware pipeline rather than two interchangeable providers:
...
4. A missing or unreliable native page is rendered by PyMuPDF and sent to the configured PaddleOCR mode.
5. If PaddleOCR fails, any usable native fragments remain in the Canonical Parse with a page-level warning. An image-only page remains `OCR unavailable`; PyMuPDF is not presented as having recovered text that was never encoded in the PDF.

PyMuPDF and PaddleOCR remain behind the same `material_parse` capability and do not add separate model-visible tools, provider selectors, or normal-turn prompt instructions. Parse Identity records the pinned PyMuPDF version and, for OCR pages, the PaddleOCR version and mode.
...
The first release fixes the active Configured OCR Capability to local PaddleOCR. Ordinary OCR is the default execution mode; PP-StructureV3 is activated only when the selected page requires complex layout, table, formula, seal, or image-aware parsing. These are modes of one configured capability, not separate providers or an ordered Provider Chain. PaddleOCR, PaddlePaddle, optional acceleration libraries, and model files are not embedded in the main installer. Standard Access requires review and confirmation of their pinned installation before first use; Full Access may install them for the current task without another prompt while showing the same package and version details.

On invocation, the Host records the PaddleOCR version and mode, duration, source page or image mapping, warnings or failure, and confirms that processing remained local. OCR-derived text enters the Canonical Parse as provider-derived blocks with stable source references. Any future external OCR requires an explicit design and User enablement because local materials or rendered images may be uploaded.

PaddleOCR and its selected local models run only in the Utility Worker Pool. They may remain warm for bounded reuse, but they do not load into Electron Main or a Project Agent Worker and receive no Provider credentials or unrelated Project content.

When the runtime or model is unavailable, or the selected mode fails, the affected pages remain visibly `OCR unavailable`; the Agent may continue with native or partial content retained by the PDF Parse Pipeline and other readable materials, and the User may repair the local installation or manually retry. PyMuPDF is a native parser and renderer, not a second OCR provider. The app does not silently switch to MinerU, Unlimited-OCR, an external API, or another OCR engine.
...
- PaddleOCR runtime, model, active mode, and availability.
- PDF Parse Pipeline diagnostics, including pinned PyMuPDF and PaddleOCR runtime/model availability.
...
The backup excludes the Project registry, Project names, paths, ids, references, Materials, Outputs, Canonical Parse, Project Context, Project Memory, artifact registry, Physical Model Context, Project-scoped Dream Cutoffs, Dream Carryover, pending review queues, resumable Dream or Investment Reflection state, local caches, and operational logs. Confirmed Dream results that already became Long-term Memory or Cognitive Evolution History remain included through those durable records. Protected Credentials are never exported, and restored Provider or authenticated-capability configuration requires fresh credential setup.
...
- Invoke local PaddleOCR when scanned PDF/image content needs OCR, use ordinary OCR by default and PP-StructureV3 for complex pages, with no automatic switch to another OCR provider.
...
- E2E smoke test: open project, parse fixture materials, generate a source-referenced task-specific Output, invoke a fixture document Skill to create an edited copy, refresh Context, capture Project Memory candidate, run `/dream`, approve proposal, and commit memory.
- Capability tests should use fixture Skills, Extensions, and mock providers where possible. CI should not require real external OCR APIs or Microsoft Office installation, but the app must support user-configured capabilities and clearly report unavailable dependencies locally.
