  ...
  projectTurns(threadId: string): TrajectoryTurn[] {
    ...
    for (const event of this.loadEvents(threadId)) {
      ...
      if (event.event === "turn.started") {
        ...
        turns.set(event.turnId, { ...turn, status: "completed", assistantText: event.payload.message, profile: event.payload.profile, usage: event.payload.usage, ...(event.payload.citations === undefined ? {} : { citations: event.payload.citations }), ...(event.payload.contextUsage === undefined ? {} : { contextUsage: event.payload.contextUsage }), ...(event.payload.latencyMs === undefined ? {} : { latencyMs: event.payload.latencyMs }), ...(event.payload.recalledStateEstimatedTokens === undefined ? {} : { recalledStateEstimatedTokens: event.payload.recalledStateEstimatedTokens }), lastSequence: event.sequence });
